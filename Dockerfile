...
RUN apt-get -q update; apt-get -qy upgrade && \
  ...
  echo 'PLUGINS = ["netbox_qrcode","netbox_interface_synchronization","netbox_reorder_rack","netbox_topology_views","netbox_attachments","netbox_inventory","netbox_floorplan"]' >> /etc/netbox/config/plugins.py && \
  # Cleanup
  apt-get purge \
      --yes -qq \
      git && \
