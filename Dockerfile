...
RUN apt-get -q update; apt-get -qy upgrade && \
  ...
  echo 'PLUGINS = ["netbox_qrcode","netbox_topology_views","netbox_inventory","netbox_floorplan","netbox_documents","netbox_lifecycle","netbox_otp_plugin","nb_service"]' >> /etc/netbox/config/plugins.py && \
  # Cleanup
  apt-get purge \
      --yes -qq \
      git && \
