...
FROM netboxcommunity/netbox:v4.3.2@sha256:06efb429e1c5d12a557b0146175e2c7dc90c24ccc56095fba19704542e3e4e55
...
RUN apt-get -q update; apt-get -qy upgrade && \
  ...
  echo 'PLUGINS = ["netbox_qrcode","netbox_interface_synchronization","netbox_topology_views","netbox_attachments","netbox_inventory","netbox_floorplan","netbox_contract","netbox_documents","netbox_lifecycle","netbox_otp_plugin","nb_service"]' >> /etc/netbox/config/plugins.py && \
  # Cleanup
  apt-get purge \
      --yes -qq \
      git && \
